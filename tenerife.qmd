...
<note>Recordau abans de seguir treballant fer `PULL`!! (i comprovar si hem penjat res també abans des push) Per evitar conflictes i branques</note>
...
<gra>Ara que ho pens sa regressió lineal no és inferència?</gra>

Calcularem la recta de regressió on la variable independent sirà la longitud en 2d, i la variable dependent, sirà la longitud en 3 dimensions de la suma dels afluents.
...
plot(longituts_3d~longituts_2d, main = "Recta de Regressió Lineal", xlab = "Longitud 2d", ylab = "Longitud 3d",col="blue", pch = 19)+abline(model, col = "red", lwd = 2)
```
<gra>Se pot llevar això de `integer(0)`?</gra>
